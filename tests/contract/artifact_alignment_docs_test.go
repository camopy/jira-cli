//go:build docs_alignment

// These tests assert that the README, the embedded agent guide, and the
// docs site stay in step with the live command surface. They read doc
// content, not code, so they are gated behind the `docs_alignment` build
// tag and excluded from the default `go test ./...` (and the release
// preflight). Run them with `mise run test:docs`.
package contract
...
// agentGuideDir (declared in guide_flag_surface_test.go). Order is not
// significant — these checks only use strings.Contains / does-not-contain.
