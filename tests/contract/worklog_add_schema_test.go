...
`), 0o600); err != nil {
	...
	cmd := exec.Command("go", "run", "../../cmd/jira", "--config", path, "--json", "worklog", "add", "PROJ-1", "--time-spent", "1d", "--dry-run", "--no-input")
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("worklog add error = %v\n%s", err, out)
	}
...
func TestWorklogAddAcceptsStartedAndJSONInput(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "../../cmd/jira", "--json", "worklog", "add", "PROJ-1", "--dry-run", "--no-input", "--json-input", input)
	out, err := cmd.CombinedOutput()
	if err != nil {
