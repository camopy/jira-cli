...
var taxonomyRegistry = map[string]struct {
	...
	"jq_eval_failed":            {"validation", 3},
	"rank_rejected":             {"validation", 3},
	"rank_partial":              {"validation", 3},
...
func taxonomyCases() []struct {
	...
	return []struct {
		...
		{"rank rejected", &jira.RankRejectedError{Wrapped: &jira.APIError{StatusCode: 400, Message: "no board"}}, "rank_rejected", "Ranking needs a Jira Software board with backlog ordering — check the project has one and that the anchor issue exists on it.", false},
		{"rank partial", &jira.RankPartialError{Failed: []jira.RankEntry{{Issues: []string{"PROJ-2"}, Status: 400}}}, "rank_partial", "Re-run the command with just the issues named in the error once the cause is fixed; the others already ranked.", false},
		{"command unknown", NewCLIInputError(InputCommandUnknown, `unknown command "nope"`), "command_unknown", "Check the command's spelling, or run `--help` on its parent to see the ones it accepts.", true},
