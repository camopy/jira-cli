...
- With transition-screen fields (e.g. resolution) and/or an ADF comment: `jira issue transition KEY Done --json-input payload.json` where the payload carries `fields` (either payload shape) and an optional ADF `comment` key. Valid `resolution` names vary per tenant — discover them with `jira cache resolutions --output=json` (→ `cache_metadata`) instead of guessing.
