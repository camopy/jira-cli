...
- Purge a credential whose profile was already deleted from config: `jira auth logout <profile> --base-url <site>` — the keychain entry is keyed by site host + profile name, so the flag supplies the half config no longer holds; without it a deleted/unknown profile is refused (`profile_not_defined`).
