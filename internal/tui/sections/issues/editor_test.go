...
func TestOpenCommentUsesExternalEditorWhenConfigured(t *testing.T) {
	t.Setenv("JIRA_EDITOR", "true")
	w := &callRecorder{}
	m := newVerbModel(t, fakeServices{issue: fakeIssueSvc{writes: w}})
	cmd := m.openComment()
	if cmd == nil {
		t.Fatal("expected an editor cmd when JIRA_EDITOR is set")
	}
	if m.ctrl.Active() {
		t.Error("in-modal comment opened despite a configured editor")
	}
}

func TestOpenCommentFallsBackToTextarea(t *testing.T) {
	...
	if !m.ctrl.Active() || m.ctrl.Mode() != action.ModeComment || !m.ctrl.Multiline() {
		t.Error("modal comment not open as a multiline action")
	}
}
...
func TestPasteRoutesIntoOpenAction(t *testing.T) {
	...
	if got := m.ctrl.Text(); got != "pasted summary" {
